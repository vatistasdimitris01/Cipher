...
const renderMath = (tex, display) => {
  try { return katex.renderToString(tex, { displayMode: display, throwOnError: false, output: "html" }); } catch {}
  ...
  const addServer = async () => {
    ...
    try {
      await FS.mcpRef(uid).add({
        ...
        description: form.description.trim(), authToken: form.authToken.trim(),
        enabled: true, createdAt: firebase.firestore.FieldValue.serverTimestamp(),
      });
  ...
  const inp = (field,placeholder,type="text") => (
    <input type={type} value={form[field]} placeholder={placeholder}
      ...
      style={{display:"block",width:"100%",background:"var(--bg-elev-2)",border:"1px solid var(--border)",
        borderRadius:9,padding:"9px 12px",color:"var(--fg)",fontSize:13.5,marginBottom:10,outline:"none",transition:"border-color 150ms"}}
      ...
      <div style={{maxWidth:820,margin:"0 auto",padding:"0 28px 60px"}}>
        <div style={{marginBottom:28,padding:"20px 22px",background:"var(--bg-elev)",border:"1px solid var(--border)",borderRadius:16}}>
          ...
          <p style={{fontSize:13.5,color:"var(--fg-muted)",marginBottom:14,lineHeight:1.6}}>
            Cipher connects to MCP servers over HTTP/SSE. Your server must implement the <strong style={{color:"var(--fg)"}}>Model Context Protocol</strong>.
          </p>
          <div className="codeblock" style={{margin:0}}>
            <div className="codeblock-head"><span className="codeblock-lang">json</span></div>
            <pre style={{margin:0,padding:"12px 16px",fontSize:12.5,lineHeight:1.6}}><code>{MCP_EXAMPLE}</code></pre>
          </div>
        </div>
        {!showForm ? (
          <button onClick={()=>setShowForm(true)}
            style={{display:"flex",alignItems:"center",gap:10,padding:"12px 18px",borderRadius:12,border:"1px dashed var(--border-strong)",
              color:"var(--fg-muted)",fontSize:14,width:"100%",justifyContent:"center",background:"transparent",cursor:"pointer",
              transition:"background 150ms,color 150ms",marginBottom:24}}
          ...
          <div style={{background:"var(--bg-elev)",border:"1px solid var(--border-strong)",borderRadius:16,padding:"22px",marginBottom:24,animation:"pop 200ms ease"}}>
            ...
            <div style={{position:"relative"}}>
              <input type={showToken.new?"text":"password"} value={form.authToken} placeholder="Auth token (optional)"
        ...
        {servers.length===0&&!showForm&&(
          <div style={{textAlign:"center",color:"var(--fg-dim)",fontSize:13.5,padding:"40px 0"}}>No MCP servers added yet.</div>
        )}
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {servers.map(s=>(
            <div key={s.id} style={{background:"var(--bg-elev)",border:"1px solid "+(s.enabled?"var(--border-strong)":"var(--border)"),borderRadius:14,padding:"16px 18px",opacity:s.enabled?1:0.6,transition:"opacity 200ms,border-color 200ms"}}>
              <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:14}}>
                <div style={{display:"flex",alignItems:"center",gap:12,flex:1,minWidth:0}}>
                  <div style={{width:38,height:38,borderRadius:10,background:"var(--accent)",color:"var(--accent-fg)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                ...
                <div style={{display:"flex",gap:8,flexShrink:0,alignItems:"center"}}>
                  ...
                  <button className={s.enabled?"btn-ghost sm":"btn-primary sm"} onClick={()=>toggleServer(s)}>{s.enabled?"Disable":"Enable"}</button>
                  <button style={{background:"none",border:"none",color:"var(--fg-muted)",cursor:"pointer",padding:4,borderRadius:6,display:"flex",alignItems:"center"}}
                    ...
                    aria-label="Delete"><Icon name="trash" size={15}/></button>
                </div>
              </div>
              {s.authToken&&(
                <div style={{marginTop:10,display:"flex",alignItems:"center",gap:8,borderTop:"1px solid var(--border)",paddingTop:10}}>
                  ...
                  <code style={{fontSize:12,fontFamily:"var(--font-mono)",color:"var(--fg-muted)",letterSpacing:"0.05em"}}>{showToken[s.id]?s.authToken:"••••••••••••••••"}</code>
